...
export async function main(ns) {
  ...
  if (ns.args.length !== 3) {
    ...
    ns.tprint("USAGE: run stocker-trader-pro.js MAX_CAP ENABLE_SHORT AGGRESSION");
    ns.tprint("EXAMPLE: run stocker-trader-pro.js 1e12 true 0.7");
    return;
  }
  ...
  ns.tprint("=================================================");
  ns.tprint("PRO STOCK TRADER v3.1 — REGIME AWARE");
    ...
    for (const stockData of ranked) {
      ...
      if (stockData.pos.l > 0 && forecast < SELL_THRESHOLD) {
        ...
        if (sellPrice > 0) {
          ...
          ns.toast(`💰 STOCK SOLD: ${stockData.sym} | Profit: ${profit >= 0 ? '+' : ''}${ns.formatNumber(profit)}`, "success", 5000);
      ...
      if (ENABLE_SHORT && stockData.pos.s > 0 && forecast > SELL_THRESHOLD) {
        ...
        if (coverPrice > 0) {
          ...
          ns.toast(`💰 SHORT COVERED: ${stockData.sym} | Profit: ${actualProfit >= 0 ? '+' : ''}${ns.formatNumber(actualProfit)}`, "success", 5000);
    ...
    if (params.exposure === 0) {
      ...
      if (candidates.length === 0) {
        ...
        for (const candidate of candidates) {
          ...
          const currentPos = ns.stock.getPosition(candidate.sym)[0];
          ...
          if (buyPrice > 0) {
            ...
            actions.push(`❌ BUY FAILED ${candidate.sym} | Shares: ${shares} | Price: $${price.toFixed(2)} | Reason: API rejection`);
          }
        }
      }
    }
